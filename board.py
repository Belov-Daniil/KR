...
class CheckersBoard:
    ...
    def draw_starting_position(self):
        for row in range(8):
            for col in range(8):
                if get_cell_color(row, col) == "black" and (row + col) % 2 != 0:
                    if row < 3:
                        checker_color = "white"
                    elif row > 4:
                        checker_color = "blue"
    ...
    def find_queen_moves(self, row, col, row_dif, col_dif):
        ...
        while 8 > ch_col >= 0 and 8 > ch_row >= 0:
            if self.cells[ch_row][ch_col] is None and get_cell_color(ch_row, ch_col) == 'black':
    ...
    def is_game_over(self, color):
        ...
        if is_game_over:
            if is_lose:
                message = 'Вы програли у бота, советую не думать, тогда норм получится\nНачнём заново?'
            else:
                message = 'Вы выиграли у бота, у меня это с третьего раза получилось\nНачнём заново?'
